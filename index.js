...
bot.login("NzMxNDQ4OTA4ODEwODEzNTcy.Xw2cXA.Dl0In1F4zRz51v5aq6twLpPHeFM");
    ...
    if (chan.name.startsWith("ticket")) {
        await db.findOne({ channel: chan.id }, async (a, b) => {
            if (a) {
             ...
             if (!b) return message.reply('Tidak ada hasil.');

             else {
                 await db.deleteOne({ user: b.user }, (c) => {
                     chan.send('Closed.');
                     chan.guild.channels.cache.get(chan.id).delete();
                 })
             }
        });
    } else {
        message.reply('Ini bukan channel ticket!');
    }
}
